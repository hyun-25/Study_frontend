...
function question(n) {
    let result = 2; // 가장 작은 자연수는 2부터 시작합니다.

    while (result <= n) {
        if (n % result === 1) {
            // 나머지가 1이면 해당 값이 정답입니다.
            return result;
        }
        result++;
    }

    // while문을 빠져나가면서 정답을 찾지 못한 경우
    // 이 부분에 대한 예외처리나 디폴트 값 설정을 할 수 있습니다.
    return -1; // 예를 들어, -1을 리턴하거나 다른 방식으로 처리할 수 있습니다.
}
